...
<div class="container">
		...
		<h3 style="display:flex; flex-direction:column; align-items:center;" >所有商品資料 - commodity_page.jsp</h3>
	...
	<table  class="table  vertical-align table-bordered">
		...
		<c:forEach var="commodityVO" items="${list}" begin="<%=pageIndex%>"
			...
			<tr>
				...
				<td><img class="com_pic" src="<%=request.getContextPath()%>/commoditypicservlet?com_id=${commodityVO.com_id}"></td>
				<td><a href="${commodityVO.com_download}">Download</a></td>
				<td>${commodityVO.com_price}</td>
				<td>${commodityVO.com_detail}</td>
				<td>${commodityVO.trandate}</td>
				<td>${commodityVO.com_status}</td>
				<td>${commodityVO.e_score}</td>
				<td>${commodityVO.e_pol_statistics}</td>
				<td style="width:150px;">${commodityVO.e_des}</td>
				<td>
					<FORM METHOD="post"
