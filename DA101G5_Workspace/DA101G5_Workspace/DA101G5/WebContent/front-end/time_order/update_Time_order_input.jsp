...
//Time_orderServlet.java (Concroller) 存入req的time_orderVO物件 (包括幫忙取出的time_orderVO, 也包括輸入資料錯誤時的time_orderVO物件)
%>

<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"/>
<title>Time_order資料修改 - update_time_order_input.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>Time_order資料修改 - update_time_order_input.jsp</h3>
...
<table>
	<tr>
		<td>Time_order編號:<font color=red><b>*</b></font></td>
		<td><%=time_orderVO.getTime_order_id()%></td>
	</tr>
	<tr>
		<td>teacher_id:</td>
		<td><input type="TEXT" name="teacher_id" size="45" value="<%=time_orderVO.getTeacher_id()%>" /></td>
	</tr>
	<tr>
		<td>course_order_id:</td>
		<td><input type="TEXT" name="course_order_id" size="45"	value="<%=time_orderVO.getCourse_order_id()%>" /></td>
	</tr>
	<tr>
		<td>language_id:</td>
...
<!-- 		<td><select size="1" name="dept_id"> -->
<%-- 			<c:forEach var="deptVO" items="${deptSvc.all}"> --%>
<%-- 				<option value="${deptVO.dept_id}" ${(time_orderVO.dept_id==deptVO.dept_id)?'selected':'' } >${deptVO.dname} --%>
<%-- 			</c:forEach> --%>
<!-- 		</select></td> -->
<!-- 	</tr> -->
...
<input type="hidden" name="time_order_id" value="<%=time_orderVO.getTime_order_id()%>">
  ...
  try {
	  start_time = time_orderVO.getStart_time();
	   ...
	   java.sql.Timestamp  start_time1 = null;
	    ...
	    start_time=new java.sql.Timestamp(start_time1.getYear(),start_time1.getMonth(),start_time1.getDate(),0,0,0,0);
  }
%>
  ...
  try {
	  end_time = time_orderVO.getStart_time();
	   ...
	   end_time1 = new java.sql.Timestamp(System.currentTimeMillis());
	    end_time=new java.sql.Timestamp(end_time.getYear(),end_time1.getMonth(),end_time1.getDate(),0,0,0,0);
  }
%>
...
$('#f_date2').datetimepicker({
   ...
   step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
   format:'Y-m-d H',         //format:'Y-m-d H:i:s',
