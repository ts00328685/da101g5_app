	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front-end/card/select_page.jsp");
				...
				String url = "/front-end/card/listOneCard.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllCard.jsp的請求
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("cardVO", cardVO);         // 資料庫取出的cardVO物件,存入req
				String url = "/front-end/card/update_card_input.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/front-end/card/listAllCard.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("update".equals(action)) { // 來自update_card_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front-end/card/update_card_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
        ...
        if ("insert".equals(action)) { // 來自addCard.jsp的請求  
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("cardVO", cardVO); // 含有輸入格式錯誤的cardVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/front-end/card/addCard.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url = "/front-end/card/listAllCard.jsp";
