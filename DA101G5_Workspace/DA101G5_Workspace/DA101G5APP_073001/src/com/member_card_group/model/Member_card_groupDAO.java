...
//	String passwd = "123456";
	...
	private static final String GET_ALL_BY_MEMBER_ID =
			"select member_card_group_id, teacher_id, teacher_card_group_name from member_card_group "
			+ "left join teacher_card_group on teacher_card_group.teacher_card_group_id = member_card_group.teacher_card_group_id "
			+ "where member_card_group.member_id = ?";
	private static final String GET_TEACHER ="select mem_name, mem_nick from member left join teacher on member.member_id = teacher.member_id "
			+ "where teacher.teacher_id = ?";
	private static final String GET_QTY = "select count(*) from member_card_table where member_card_group_id = ?";
...
// JNDI
	...
	@Override
	public List<Member_card_groupVO> getAllByMember_id(String member_id) {
		...
		List<String> teacher_idList = new ArrayList<String>();
...
//			con = DriverManager.getConnection(url, userid, passwd);
			...
			//get member_card_group_id, teacher_id, teacher_card_group_name
			...
			while(rs.next()) {
				...
				member_card_groupVO.setMember_card_group_id(rs.getString(1));
				teacher_idList.add(rs.getString(2));
				member_card_groupVO.setTeacher_card_group_name(rs.getString(3));
				list.add(member_card_groupVO);
			}
			...
			for(int i=0; i<list.size(); i++) {
				pstmt = con.prepareStatement(GET_TEACHER);
				pstmt.setString(1, teacher_idList.get(i));
				rs = pstmt.executeQuery();
				rs.next();
				list.get(i).setTeacher_name(rs.getString(1));
				rs.close();
				pstmt.close();
			}
			...
			//get qty
			for(int i=0; i<list.size(); i++) {
				pstmt = con.prepareStatement(GET_QTY);
				pstmt.setString(1, list.get(i).getMember_card_group_id());
				rs = pstmt.executeQuery();
				rs.next();
				list.get(i).setQty(rs.getInt(1));
				rs.close();
				pstmt.close();
			}
